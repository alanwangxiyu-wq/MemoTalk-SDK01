...
初始化SDK。必须在使用任何其他SDK方法之前调用一次，通常在您的`AppDelegate`中。
...
-   **返回值**：包含已发现的`Device`对象数组的`Result`
...
### `resumeRecording(device: Device) async -> Result<Void, PlaudError>`
...
### `clearAllRecordings(device: Device) async -> Result<Void, PlaudError>`
...
-   **参数**：
    ...
    -   `progressCallback`：接收下载进度更新（0-100）的闭包
-   **返回值**：包含已下载文件URL的`Result`

### `deleteRecording(device: Device, recording: Recording) async -> Result<Void, PlaudError>`

从设备删除录音。

### `getStorageInfo(device: Device) async -> Result<(free: Int64, total: Int64), PlaudError>`

获取设备的存储信息。

-   **返回值**：包含（可用空间，总空间）元组的`Result`

### `uploadRecordingToCloud(recording: Recording, filePath: String) async -> Result<String, PlaudError>`
...
-   **参数**：
    -   `firmwareFile`：固件文件URL
    -   `progressCallback`：接收更新进度（0-100）的闭包

### `syncDeviceTime(device: Device) async -> Result<Void, PlaudError>`
...
### `disableRealtimeTranscription(device: Device) async -> Result<Void, PlaudError>`
...
### `summarizeRecording(device: Device, recording: Recording) async -> Result<String, PlaudError>`
